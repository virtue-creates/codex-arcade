...
function renderFaultList() {
  const faultButtons = initialEdges.filter((edge) => edge.broken).map((edge) => {
    const from = stationById(edge.from);
    const to = stationById(edge.to);
    const repaired = state.repaired.has(edge.id);
    const button = document.createElement("button");
    button.className = `fault-button ${repaired ? "is-repaired" : ""}`;
    button.type = "button";
    button.dataset.edgeId = edge.id;
    button.disabled = repaired || state.mode !== "play" || state.repairsLeft <= 0;

    if (repaired) {
      button.textContent = `${from.name} - ${to.name}: REPAIRED`;
    } else if (state.mode === "play") {
      const outcome = getRepairOutcome(edge.id);
      const count = outcome.newStations.size;
      button.textContent = `${from.name} - ${to.name}: +${count} station${count === 1 ? "" : "s"}`;
    } else {
      button.textContent = `${from.name} - ${to.name}: FAULT`;
    }

    button.addEventListener("mouseenter", () => previewEdge(edge.id));
    button.addEventListener("mouseleave", clearPreview);
    button.addEventListener("focus", () => previewEdge(edge.id));
    button.addEventListener("blur", clearPreview);
    button.addEventListener("click", (event) => {
      event.preventDefault();
      repairEdge(edge.id);
    });
    button.setAttribute("onclick", `window.metroMenderRepair && window.metroMenderRepair("${edge.id}")`);
    return button;
  });

  elements.faultList.replaceChildren(...faultButtons);
}

function handleFaultListClick(event) {
  const button = event.target.closest("[data-edge-id]");
  if (!button || button.disabled) return;

  event.preventDefault();
  repairEdge(button.dataset.edgeId);
}
...
window.metroMenderRepair = repairEdge;
...
function init() {
  ...
  elements.faultList.addEventListener("click", handleFaultListClick);
