...
```txt
docs/03-cabinets/cabinet-02/cabinet-02-design-intent-report.md
```
...
Cabinet 02 Studioから、HQ Direction対応版の報告を受領した。

主な変更:

- 120秒から90秒へ短縮。
- 故障5本/修理5回から、故障7本/修理4回へ変更。
- `CENTRAL` 駅と `MAJOR` 駅を見える化。
- ホバー時に修理効果予測を表示。
- 修理後にスコア、復旧率、復旧駅数の変化を表示。
- リザルトに主要駅と孤立駅を追加。

本部判断:

- HQ Directionの中心課題だった「選択の重みを作る」には対応している。
- 次は本部リポジトリに改善版を取り込み、静的QAと人間QAを行う。
