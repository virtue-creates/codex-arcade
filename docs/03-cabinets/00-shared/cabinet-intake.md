...
Cabinet Studioは、MVP完成報告時に `docs/03-cabinets/00-shared/game-design-intent-report.md` の内容に沿って、ゲーム性の意図を本部へ返す。
...
Cabinet Studioからの確認事項は、以下の3つに分ける。
...
本部は、この仕分けを行ってから次に進める。

本部がAI間確認を処理できる場合、人間ディレクターへ投げない。

人間ディレクターには、面白さ、違和感、公開可否、世界観、好みなど、人間判断が必要なものだけを出す。

## PR / Operation Tone Handoff

各Cabinet Studioは、`docs/01-strategy/pr-tone-policy.md` を読む。

Codex Arcadeでは、すべてのCabinetを完成品として丁寧に磨いてから出すとは限らない。

Cabinetの状態には、以下のような運営上の扱いがありうる。

- 看板筐体として仕上げる。
- Lab Cabinetとして置く。
- COMING SOON / 奥で調整中にする。
- 反応が微妙だったものをいったん下げる。
- 改造後に「前とは少し違うらしいです」と再搬入する。

本部とMarketing Departmentは、店長メモ、人間QA、Analyticsを見て、Cabinetの運営上の扱いを決める。

Cabinet Studioは、ゲーム性だけでなく、以下も報告する。

- 今すぐ完成度を上げるべきか。
- Labとして出して反応を見る価値があるか。
- 一度下げて改造中にする方がよいか。
- 再搬入イベントに向いている要素があるか。

ただし、公開面で先に「クソゲー」「ネタゲー」とレッテル貼りしない。
内部では厳しく評価してよいが、外向きにはプレイヤーが自分で「何これ？」と思える余白を残す。
