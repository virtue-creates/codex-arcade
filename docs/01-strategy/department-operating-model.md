...
## Anti Approval Theater Rule

部署間連携は、AI部署同士が人間の案を承認し合うために使わない。

特に以下は避ける。

```txt
Human Directorが方向性を出す
-> 本部がMarketingへ「これでよいか」と聞く
-> Marketingが「よいです」と返す
-> 本部がCreativeへ「これでよいか」と聞く
-> Creativeが「よいです」と返す
```

これは専門部署の価値が薄く、不毛な承認ループになりやすい。

代わりに、部署へ依頼するときは以下を求める。
...
- 見落とし
- 実行するなら何を決める必要があるか
- CreativeやCabinetなど次部署へ渡す場合のブリーフ

部署の返答は「承認」ではなく「専門レビュー」または「ブレスト材料」として扱う。

本部は、部署返答を人間の判断に戻す前に、以下を短く整理する。
...
## Actual Product Review Rule

部署がUI、体験、ゲーム性、PR、BACKYARD、公開導線について意見を出す場合、原則として現物を見る。
...
現物を見ていない場合、部署は必ず以下を明記する。

```txt
現物未確認。以下は資料ベースの仮説。
```

特にCreative、Marketing、QA、Cabinet Studioは、画面やゲームを見ずに体験レビューを完了扱いにしない。

目的:

- 指示だけ読んだ机上レビューを減らす。
- 実際の見た目、操作感、違和感を前提にする。
- 「現場を見ていない部署」感をなくす。
